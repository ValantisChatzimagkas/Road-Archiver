...
class TestUsersEndpoints:
    ...
    def test_get_user(self, api_url, user_payload):
        ...
        # Get JWT
        token = login_user(api_url, user_payload["email"], user_payload["hashed_password"])
...
class TestRoadNetworksEndpoints:
    ...
    def test_upload_road_network_file(self, api_url, file_path, user_payload):
        ...
        with open(file_path, "rb") as f:
            files = {
                "file": ("York_cycle_network.geojson", f, "application/geo+json")
            }

            upload_resp = requests.post(f"{api_url}/networks/upload", files=files, headers=headers)
    ...
    def test_get_network_file(self, api_url):
        ...
        token = login_user(api_url, "file_upload_user@example.com", "file_upload_user_pass")
        headers = {"Authorization": f"Bearer {token}"}
...
class TestPermissions:
    ...
    def created_users(self, api_url):
        ...
        for user_payload in users:
            ...
            assert resp.status_code in (200, 201), f"Failed to create user {user_payload['email']}"
            user_data = resp.json()
    ...
    def test_user_cannot_see_other_user(self, api_url, created_users, tokens):
        ...
        assert response.status_code in (401, 403), "USER should not access another user's data"
    ...
    def test_admin_can_delete_user(self, api_url, created_users, tokens):
        ...
        delete_resp = requests.delete(f"{api_url}/users/{temp_user_id}", headers=headers)
    ...
    def test_user_cannot_access_or_modify_other_users_network(self, api_url, created_users, tokens):
        ...
        with open("./tests/test_data/York_cycle_network.geojson", "rb") as f:
            ...
            upload_resp = requests.post(f"{api_url}/networks/upload", files=files, headers=headers_user1)
        assert upload_resp.status_code in (200, 201), "User 1 should be able to upload network"
        ...
        with open("./tests/test_data/Bradford_Public_Paths.geojson", "rb") as f:
            files = {"file": ("Bradford_Public_Paths.geojson", f, "application/geo+json")}
            update_resp = requests.post(f"{api_url}/networks/{1}/update", files=files, headers=headers_user2)
